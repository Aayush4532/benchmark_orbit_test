...
func respondSaleError(c *gin.Context, err error) {
	log.Printf("sale action failed: event=%s action=%s error=%v", c.Param("eventId"), c.Request.URL.Path, err)
	...
	c.JSON(http.StatusInternalServerError, gin.H{"error": "sale operation failed"})
}
